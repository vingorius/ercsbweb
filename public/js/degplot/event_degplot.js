...
.origin(Object)
.on("drag", dragLever).on("dragend", dragEnd);

var allColors = function()	{
	var all_color = d3.selectAll("stop")[0];
	var result = [];

	for(var i = 0, len = all_color.length ; i < len ; i++)	{
		if(all_color[i].id.indexOf("end_color_list") > -1)	{
			result.push(all_color[i]);
		}
	}
	return result;
}

var findColor = function(_target, _color, _now_color)	{
...
var whatInAllColors = function(_all_color, _now_color)	{
	for(var i = 0, len = _all_color.length ; i < len ; i++)	{
		var target = "color_list_" + _utils.getNum($(_all_color[i]).attr("id"));

		findColor(target, _all_color[i], _now_color);			
	}
}

var setSelectedBtn = function(_target)	{
