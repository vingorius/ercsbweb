define("population/comutationplot/navigation/event_comutationnavigation", ["utils", "size", "population/comutationplot/vo_comutationplot"], function(_utils, _size, _VO)	{
	return 	function(_data) {
		...
		var getInputValue = function()    {
			var value = $("#comutationplot_scale_input").val();

			return Number(value.substring(0, value.length - 1));
		}

		var changeInputValue = function(_sign, _value)   {
			$("#comutationplot_scale_input").val(Number(getInputValue() + ((_value || 10) * _sign)) + "%");
		}
		...
		var getScale = function()	{
			...
			d3.selectAll("#comutationplot_heatmap_border_rect").attr("width", now);
		...
		var mOver = function(_d)	{
			(/initial/).test(this.id) ? tooltip.show(this, "<b>Initialization</b>", "rgba(178, 0, 0, 0.6)") : 
			(/down/).test(this.id) ? tooltip.show(this, "<b>Scale down </br> minimum(100%)</b>", "rgba(178, 0, 0, 0.6)") : 
