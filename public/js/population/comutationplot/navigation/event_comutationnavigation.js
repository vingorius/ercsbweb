...
define("population/comutationplot/navigation/event_comutationnavigation", ["utils", "size", "population/comutationplot/vo_comutationplot"], function(_utils, _size, _VO)	{
	return 	function(_data) {
		...
		var changeInputValue = function(_sign, _value)   {
			...
			.val((getInputValue() + ((_value || 10) * _sign)) + "%");
		}
		...
		var getScale = function()	{
			...
			d3.selectAll("#comutationplot_heatmap_border_rect, .comutationplot_groups, .comutationplot_sample, .comutationplot_heatmap")
			.attr("width", now);
		...
		var getInit = function(_d)	{
			var init_y = _utils.ordinalScale(_VO.VO.getInitGene(), 0, _VO.VO.getInitHeight());
			var init_x = _utils.ordinalScale(_VO.VO.getInitSample(), 0, _VO.VO.getInitWidth());

			_VO.VO.setGene(_VO.VO.getInitGene());
			_VO.VO.setSample(_VO.VO.getInitSample());

			sampleRedraw(_VO.VO.getInitWidth(), init_x);
			groupRedraw(_VO.VO.getInitWidth(), init_x);
			comutationRedraw(_VO.VO.getInitWidth(), init_x, init_y);
			
			getInputValue() === 100 ? changeInputValue(-1, 0) : changeInputValue(-1, (getInputValue() - 100));
			...
			d3.select(".comutationplot_heatmap").attr("width", _VO.VO.getInitWidth());
			$("#comutationplot_heatmap").width(_VO.VO.getInitWidth());
