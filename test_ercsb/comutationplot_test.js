...
describe('Co Mutation Plot Test Suite:', function() {
    it('path가 존재하여야 한다.', function(done) {
        request(host)
            .get('/rest/comutationplot?cancer_type=luad&sample_id=Pat99&filter=')
            .expect(200, done);
    });
    ...
    it('JSON Data Format Check', function(done) {
        request(host)
            .get('/rest/comutationplot?cancer_type=luad&sample_id=Pat1099&filter=')
            .end(function(err, res) {
                ...
                assert.equal(8, transfer_object.data.group_list.length);
